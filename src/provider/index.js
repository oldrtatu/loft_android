  ...
  login = (username, password) => {
    return new Promise(async (resolve, reject) => {
      await loginuser(this.state.url, username, password)
        .then(res => {
          this.setState(
            {user_data: res.user, token: res.token, authenticated: true},
            () => {
        ...
        .catch(err =>
          this.setState(
            {user_data: {...this.state.user_data, error: err.message}},
            () => {
              resolve([this.state.authenticated, this.state.user_data.error]);
            },
          ),
  ...
  adddata = async (path, table, data) => {
    ...
    if (res.message) {
      ...
      if (table == 'issuesdata') {
        ...
        ob[res.id][data.equipmentType.toLowerCase()] =
          data[data.equipmentType.toLowerCase()];
        this.setState({issuesdata: ob});
      } else if (table == 'inventorydata') {
